...
def move_major_left(given_Node, step_size, goal_x, goal_y):
    return create_new_node(given_Node, step_size, 60, goal_x, goal_y)

def move_minor_left(given_Node, step_size, goal_x, goal_y):
    return create_new_node(given_Node, step_size, 30, goal_x, goal_y)

def move_straight(given_Node, step_size, goal_x, goal_y):
    return create_new_node(given_Node, step_size, 0, goal_x, goal_y)

def move_minor_right(given_Node, step_size, goal_x, goal_y):
    return create_new_node(given_Node, step_size, -30, goal_x, goal_y)

def move_major_right(given_Node, step_size, goal_x, goal_y):
    return create_new_node(given_Node, step_size, -60, goal_x, goal_y)

# # create_new_node is the main body of each of the move functions. 
# def create_new_node(given_Node, step_size, theta, goal_x, goal_y):
#     # Get the parent
...
#     cost = given_Node.Node_Cost + step_size 

#     # Calculate the new position from the movement.
#     x_pos = round((given_Node.Node_x + step_size*np.cos(np.deg2rad(given_Node.Node_theta + theta)))*2)/2 
#     y_pos = round((given_Node.Node_y + step_size*np.sin(np.deg2rad(given_Node.Node_theta + theta)))*2)/2
...
def create_new_node(given_Node, RPMs, goal_x, goal_y, obstacles):
    ...
    x_pos = round(x_points[-1])
    y_pos = round(y_points[-1])
    theta_pos = round(final_theta)
...
# Convert the angle given, in degrees, to an index from 0-359.
def angle_to_index(angle):
    # Normalize angle to the range [0, 360)
    angle = angle % 360
    
    # Compute the index by dividing the angle by 1
    return int(angle // 45)
...
@timer
def A_star_search(map, obstacles, start, end, step_size):
...
def get_final_path(visited, end_node):
    ...
    node_key = (int(current_y), int(current_x), angle_to_index(current_theta))
...
def main():
    ...
    end_x, end_y, end_theta = get_point(prompt="end", obstacles=obs_map_gray)
    ...
    # Apply A* search
    final_path_image = A_star_search(map=expanded_obstacle_map2, obstacles=obs_map_gray, start=(start_x, start_y, start_theta), end=(end_x, end_y, end_theta), step_size=step_size)
